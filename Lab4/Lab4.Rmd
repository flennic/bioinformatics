...
Finally, we create a volcano plot of the results. Note that both, red and green
points, correspond to significant genes with a fold change smaller than -5 (green)
or larger than 5 (red). Fold change describes how much a quantity changes between
an original and a subsequent measurement (here, in the contrasts between different
samples of specific tissues (?)). We care most about the red and the green points
...
# Asked questions to Krzysztof about the meaning of this specific question: 
# he wants us to plot just ONE of all the possible 144 pairs, possibly one
# that will lead us to "interesting results". 
# Problem is: 144 are obviously to much and plotting every single point 
# of the observed one will take ages to render
# --> To detect the "most interesting" pair I sampled 10k points from each 
# dataset and "quickly" plot them using GGally::ggpairs()
...
raw_data = as.data.frame( eset[id, ] )
...
plot_function <- function(df) {
  ...
  ggpairs(df) +
    theme_light()
  ggsave(filename = filename, width = 20, height = 12.95)
