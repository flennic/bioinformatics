...
## a)
Cross-Validation is used to estimate the prediction fitness of a model. To ensure that the estimate is not biased and accurate, the supplied data-set is divided into k-folds. The cross-validation iterates `k` times, where each time the `1, 2, ..., k` part is used for validation and all the others parts are used for training.
